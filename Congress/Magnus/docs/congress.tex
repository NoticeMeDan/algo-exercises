...
\begin{tikzpicture}[remember picture, overlay]
  ...
  ([xshift = +1cm,yshift = +.7cm] current page.south west) 
  rectangle
  ([xshift = -1cm,yshift = -2.2cm] current page.north east);
...
\bigskip
  {Thore Husfeldt and Riko Jacob, %Rev\GITAuthorName,
...
\bigskip
\textit{Huntington--Hill.}
Here’s how it’s done, using the Huntington–Hill method from 1911:
...
{ {\textbf Requirements } \par\medskip\noindent
  ...
  or the book’s,
  or grab one from the internet (make sure it’s correct and you acknowledge the source).

  Your code must at least pass all tests in \texttt{TestsSmall} on codeJudge.
};
\end{tikzpicture}
...
\noindent The files for this exercise are called \texttt{congress*}
in (the downloads section) of \url{https://bitbucket.org/rikj/bads-labs}.
...
\begin{enumerate}
 ...
 \item \href{https://en.wikipedia.org/wiki/Huntington\%E2\%80\%93Hill_method}{``Huntington--Hill method.'' \emph{Wikipedia: The Free
     Encyclopedia.}}
